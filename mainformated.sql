...
CREATE TABLE Coach (
    ...
    CONSTRAINT chk_TotalAvailableSeats CHECK (TotalAvailableSeats <= 50),
...
CREATE OR REPLACE PACKAGE RMS_Package AS
    ...
    FUNCTION determine_fare_multiplier (
...
--Find the minimum seat number that is not already assigned to a confirmed passenger.
    ...
    BEGIN
        ...
        FROM (
            ...
            FROM dual
            CONNECT BY LEVEL <= (SELECT TotalSeats FROM Coach WHERE CoachID = p_coach_id)
            MINUS
            SELECT SeatNumber
            FROM Passenger
            WHERE CoachID = p_coach_id AND Status = 'Confirmed'
        );
        RETURN v_seat_number;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RETURN NULL;
    END find_lowest_available_seat;
    ...
    BEGIN
        ...
        v_travel_distance := calculate_travel_distance (p_train_id, p_from_station_id, p_to_station_id);
        v_fare_multiplier := determine_fare_multiplier (p_class_type);
...
BEGIN
    ...
    passenger_names(1) := 'Nilesh';
    passenger_ages(1) := 34;
    passenger_genders(1) := 'M';

    passenger_names(2) := 'Bhaiya';
    passenger_ages(2) := 28;
